...
execute:
  echo: true
footer: "PSYCH 490.009: 2023-09-18 Monday"
---
...
*Effect sizes* are measures of how much an intervention changed some outcome. 

A *power analysis* is a tool for determining how large a sample should be in order to detect a specific effect with a specified probability. For example, let's say want to detect some effect say 80%^[And, yes fail to detect it, or have a false 'negative' 20% of the time.] of the time, how large a sample do we need?
...
A consistent finding emerging from replication studies across fields is that replication effect sizes are often *smaller* than the original. What does this mean for 'consumers' of science?
