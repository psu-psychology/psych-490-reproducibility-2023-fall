---
title: "Visualizing data from Silberzahn et al."
---
...
```{r, eval = FALSE}
osf_tbl <- osfr::osf_retrieve_file("fv8c3")
viz_file <- osfr::osf_download(osf_tbl, progress = TRUE)
```

The previous code wasn't working, so I went ahead and downloaded the file manually. :/

::: {.callout-note}

I should wrap my head around and master use of the `osfr` package.
That would make this workflow more transparent and reproducible.

:::

It is saved as a CSV in `/src/surveys/csv/CrowdstormingDataJuly1st.csv`.

```{r}
red_cards <- readr::read_csv(paste0(here::here(), "/src/surveys/csv/CrowdstormingDataJuly1st.csv"), show_col_types = FALSE)
