...
```{r}
my_hist_2 <- function(data = survey_03, var) {
  data |>
    ggplot() +
    aes(x = {{var}}) +
    geom_histogram() +
...
return_section <- function(data, var) {
  a <- knitr::knit_expand(text = c(
    "### Histogram for: `{{this_var}}`",
    "\n",
    paste0("Question: '", retrieve_long_q(var), "'"),
    "\n"),
    this_var = var)
  cat(a, sep = "\n")
  #cat(unlist(return_plot(data, var)))
}
```

Run.

```{r many-tables-lapply, results = "asis"}

these_vars <- names(survey_03)[2:12]

invisible(lapply(these_vars, return_section, data = survey_03))
```

::: {.callout-note title="Status"}

The above code is a stub. The plot portion is not yet working.
